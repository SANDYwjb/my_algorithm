 ...
 * 思路：
 *
 * 首先判断 l1 或 l2 中是否有一个节点为空，如果存在，那么我们只需要把不为空的节点接到链表后面即可。本题的思路还是很清晰的，因为l1和l2都是有序的。新建一个链表res用来存储最后的结果。因此每次比较l1和l2的头节点的大小。将较小的加入res，并将res和较小的节点所在链表都后移一位。最后还需要分别判断l1和l2是否已经遍历完。如果有没有遍历完的，直接将其加入res即可。
 *
 * **非递归解法：
 * @author junbin.wang
 * @description: TODO
