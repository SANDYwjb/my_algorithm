 ...
 * 建立哈希表构建左右括号对应关系：key左括号，value 右括号；这样查询 2 个括号是否对应只需 O(1)时间复杂度；建立栈 stack，遍历字符串 s 并按照算法流程一一判断。
 * 算法流程
 * 如果 c 是左括号，则入栈 push；
 ...
 * **解决边界问题**：
 * 1. 栈 stack 为空： 此时 stack.pop() 操作会报错；因此，我们采用一个取巧方法，给 stack 赋初值 ? ，
 * 并在哈希表 dic 中建立 key: '?'，value:'?'的对应关系予以配合。此时当 stack 为空且 c 为右括号时，可以正常提前返回 false；
 * 2. 字符串s以左括号结尾： 此情况下可以正常遍历完整个 s，但 stack 中遗留未出栈的左括号；因此，最后需返回 len(stack) == 1，以判断是否是有效的括号组合。
 * 复杂度分析
    ...
    public boolean isValid(String s) {
        ...
        for (Character c : s.toCharArray()) {
            if (map.containsKey(c)) {
                ...
                // 如果不是左括号，且栈内的上一个元素(左括号)对应的value和c不相等，则说明不是配套的右括号
                return false;
            }
        }
        //正常情况，只能保留？这个一个元素
