 ...
 * 边界条件为：dp[0]=nums[0]，只有一间房屋，则偷窃该房屋。dp[1]=max(nums[0],nums[1])，只有两间房屋，选择其中金额较高的房屋进行偷窃
 * 最终的答案即为 dp[n−1]，其中 n 是数组的长度。
 * ​正常应该使用来数组存储结果。但考虑到每间房屋的最高总金额只和该房屋的前两间房屋的最高总金额相关，因此可以使用滚动数组，
 * 在每个时刻只需要存储前两间房屋的最高总金额。
 * 时间复杂度：O(n)
 * 空间复杂度：O(1)
 *
 * @author junbin.wang
 * @date 2023/2/25下午7:51
 */
